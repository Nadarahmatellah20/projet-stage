...
Route::prefix('user')->group(function () {
    ...
    Route::post('login', [UserAuthController::class, 'login'])->name('userLogin')->middleware('throttle:5,1');
...
Route::controller(OrderController::class)->middleware(UserLoggedIn::class)->group(function () {
    ...
    Route::get('dashboard/orders/cancelled', 'DisplayCancelledOrders')->name('displayCancelledOrders');
...
// ✅ Paiement simulation
Route::match(['get', 'post'], 'dashboard/orders/{order}/invoice/pay', [InvoiceController::class, 'markAsPaid'])
...
Route::prefix('cp')->group(function () {
    ...
    Route::post('login', [AdminAuthController::class, 'admLogin'])->name('login-c')->middleware('throttle:5,1');
...
Route::prefix('cp')->middleware(AdminAuth::class)->group(function () {
    ...
    Route::post('orders/{order}/invoice',          [InvoiceController::class, 'store'])->name('storeInvoice');

    // ✅ Invoices
    Route::get('invoices', [InvoiceController::class, 'indexInvoices'])->name('indexInvoices');
    ...
    Route::get('tickets/{ticket}/unarchive',      [TicketController::class, 'UnarchiveTicket'])->name('unarchiveTicket');
