...
<head>
    ...
    <style>
        ...
        tr.prod-row td { padding:0; border-top:none; }
    </style>
</head>
    ...
    <div class="index-header">
        <div class="search-bar">
            <form action="{{ route('indexPendingOrders') }}" method="get">
                ...
                <a href="{{ route('indexPendingOrders') }}">Reset</a>
            </form>
        </div>
    </div>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @forelse ($item->OrderList as $ol)
                            @php
                                $prod = $ol->Product($ol->prod_category)?->first();
                            @endphp
                            @if ($prod)
                                <span class="prod-tag">
                                    {{ $prod->name }}
                    ...
                    <td class="col_role">
                        <span class="status {{ $item->order_status }}">{{ $item->order_status }}</span>
                    </td>

                    <td class="col_options">
                        <a class="btn view"   href="{{ route('showOrder', $item) }}">Afficher</a>
                        <a class="btn accept" href="{{ route('changeOrderStatus', $item) }}">Accepter</a>
                        <a class="btn archive" href="{{ route('archiveOrder', $item) }}">Archive</a>
                    </td>
                </tr>

            @empty
                <tr><td colspan="7" class="empty">Aucun résultat trouvé</td></tr>
            @endforelse
